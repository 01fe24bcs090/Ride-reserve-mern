...
type TrainType = "arriving" | "departing" | "both";
...
function normalizeType(value: unknown): TrainType {
...
function normalizeTrainRow(row: Record<string, unknown>) {
...
export const lookupTrain = onCall(async (request) => {
  ...
  const trainNumber = String((request.data as Record<string, unknown>).trainNumber ?? "").trim();
  if (!trainNumber) {
    throw new HttpsError("invalid-argument", "trainNumber is required");
  }
  ...
  const train = trainSnapshot.data() as Record<string, unknown>;
  if (!train.isActive) {
...
export const importTrainsFromExcel = onCall(async (request) => {
  ...
  const normalized = rows.flatMap((row, idx) => {
    ...
    return normalizeTrainRow(row as Record<string, unknown>);
  });
