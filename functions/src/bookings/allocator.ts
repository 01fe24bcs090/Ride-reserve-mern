...
async function evaluatePeakFare(scheduledHm: string): Promise<{
  isPeakHour: boolean;
  fare: number;
  multiplier: number;
}> {
...
export async function allocateBooking(input: BookingAllocationInput) {
  ...
  for (const bovDoc of bovSnapshot.docs) {
    const bov = bovDoc.data() as BovRecord;
...
export async function estimateFare(trainNumber: string, journeyType: "arrival" | "departure") {
